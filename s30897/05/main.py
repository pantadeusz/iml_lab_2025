...
def Random_Forest(X_train, X_test, y_train, y_test):
    # Używamy klasyfikatora zamiast regresora, bo problem jest binarny
    ...
    return {"accuracy": accuracy, "confusion_matrix": cm, "report": report, "predictions": y_pred}
...
def DNN(X_train, X_test, y_train, y_test, n_features):
    ...
    DNN_Model.add(Dense(units=1, activation='sigmoid'))  # sigmoid dla klasyfikacji binarnej
...
def compare_models(rf_metrics, dnn_metrics):
    ...
    print("\n" + "=" * 50)
    print("WYNIKI SIECI NEURONOWEJ (DNN)")
    print("=" * 50)
    print(f"Accuracy: {dnn_metrics['accuracy']:.4f}")
    print("\nMacierz pomyłek:")
    print(dnn_metrics['confusion_matrix'])
    print("\nRaport klasyfikacji:")
    print(dnn_metrics['report'])

    print("\n" + "=" * 50)
    print("PORÓWNANIE MODELI")
    print("=" * 50)
    if rf_metrics['accuracy'] > dnn_metrics['accuracy']:
        print(f"Wniosek: Random Forest poradził sobie lepiej (accuracy: {rf_metrics['accuracy']:.4f} vs {dnn_metrics['accuracy']:.4f})")
    elif dnn_metrics['accuracy'] > rf_metrics['accuracy']:
        print(f"Wniosek: Sieć neuronowa (DNN) poradziła sobie lepiej (accuracy: {dnn_metrics['accuracy']:.4f} vs {rf_metrics['accuracy']:.4f})")
    else:
        print("Wniosek: Oba modele osiągnęły identyczny wynik.")
    print("=" * 50)
